...
import { Button, ErrorText, LabelledInput, LabelledTextArea, Link, Paragraph, Spacer } from "@/entities";
 ...
 */
export function ContactForm({ submitEndpoint, onFormSubmitted, ...rest }: ContactFormProps) {
  ...
  const handleOnChange = (e: FormEvent<HTMLInputElement> | FormEvent<HTMLTextAreaElement>) => {
    e.persist();
  ...
  return (
    <Root data-testid={ContactForm.name} onSubmit={handleOnSubmit} {...rest}>
      ...
      <InputWrapper $hasSubmitted={status.submitted}>
        <LabelledInput label="Your email:" name="email" type="email" id="email" required={true} onChange={handleOnChange} defaultValue={inputs.email} data-lpignore={true} data-testid={`${ContactForm.name}Email`} />
        <LabelledTextArea label="Your message" name="message" id="message" required={true} onChange={handleOnChange} defaultValue={inputs.message} data-testid={`${ContactForm.name}Message`} />
